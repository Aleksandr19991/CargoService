...
`POST api/auth/login` (`AuthController`, `[AllowAnonymous]`) proxies the Resource Owner Password Credentials grant to Keycloak's token endpoint via `IIdentityProviderClient.AuthenticateAsync` and returns `{ accessToken, refreshToken, expiresIn, tokenType }`, or `401` on invalid credentials. There is no `POST api/auth/refresh` yet (`grant_type=refresh_token` against the same token endpoint) — that's the next piece of auth work. For manual testing before then, the seeded `admin@cargoservice.local` / `admin` user from `realm-export.json` works against `login`.
