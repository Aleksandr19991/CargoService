...
There is currently no endpoint that issues a token (`POST /auth/login` is not implemented yet — it will proxy to Keycloak's token endpoint rather than mint tokens itself), so every `[Authorize]`-protected endpoint is unreachable through this API alone; you can obtain a token directly from Keycloak instead (e.g. `POST {Keycloak:BaseUrl}/realms/cargoservice/protocol/openid-connect/token` with `grant_type=password`, the `identity-service` client id/secret, and the seeded `admin@cargoservice.local` / `admin` user from `realm-export.json`).
