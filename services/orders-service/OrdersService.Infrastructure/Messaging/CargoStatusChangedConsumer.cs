...
/// <summary>
/// Consumes CargoStatusChanged events published by cargo-service's outbox and projects
/// TrackingNumber/CargoStatus onto the matching Order row (read-model only — no further events
/// are published from here).
/// </summary>
public class CargoStatusChangedConsumer(
    IServiceScopeFactory scopeFactory,
    RabbitMqOptions options,
    ILogger<CargoStatusChangedConsumer> logger) : BackgroundService
{
    private const string ConsumingService = "orders-service";
    private const string PublishingService = "cargo-service";
    private static readonly TimeSpan ReconnectDelay = TimeSpan.FromSeconds(10);

    private static readonly string QueueName = RabbitMqConventions.QueueName(ConsumingService, nameof(CargoStatusChanged));
    private static readonly string DeadLetterQueueName = QueueName + RabbitMqConventions.DeadLetterSuffix;
    private static readonly string RoutingKey = RabbitMqConventions.RoutingKey(PublishingService, nameof(CargoStatusChanged));
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                logger.LogError(ex, "CargoStatusChanged consumer failed, reconnecting in {Delay}", ReconnectDelay);
    ...
    private async Task HandleMessageAsync(IChannel channel, BasicDeliverEventArgs eventArgs, CancellationToken stoppingToken)
    {
        try
        {
            var cargoStatusChanged = JsonSerializer.Deserialize<CargoStatusChanged>(eventArgs.Body.Span)
                ?? throw new InvalidOperationException("CargoStatusChanged payload deserialized to null.");

            using var scope = scopeFactory.CreateScope();
            var ordersService = scope.ServiceProvider.GetRequiredService<IOrdersService>();

            var updated = await ordersService.UpdateCargoStatusAsync(
                cargoStatusChanged.OrderId,
                cargoStatusChanged.TrackingNumber,
                cargoStatusChanged.Status,
                stoppingToken);

            if (updated)
            {
                logger.LogInformation(
                    "Updated cargo status for order {OrderId} -> {Status} ({TrackingNumber})",
                    cargoStatusChanged.OrderId, cargoStatusChanged.Status, cargoStatusChanged.TrackingNumber);
            }
            else
            {
                ...
                logger.LogWarning(
                    "CargoStatusChanged referenced unknown order {OrderId}, dropping", cargoStatusChanged.OrderId);
            }
        ...
        catch (Exception ex)
        {
            logger.LogError(
                ex,
                "Failed to process CargoStatusChanged message {MessageId}, sending to dead-letter queue",
