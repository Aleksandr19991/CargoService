...
/// <summary>
/// Обобщённый консьюмер одного события: своя очередь <c>document-service.{event}</c> с DLQ,
...
{
    private const string ConsumingService = "document-service";
    ...
    private async Task HandleMessageAsync(IChannel channel, BasicDeliverEventArgs eventArgs, CancellationToken stoppingToken)
    {
        try
        {
            ...
            // терял бы документы насовсем, упади сервис между отметкой и записью, а этот в худшем
            // случае заводит лишний комплект — лишний документ видно и можно отозвать, а
            // ненапечатанный акт обнаружится только когда он понадобится.
            if (!await inbox.TryMarkProcessedAsync(@event.EventId, EventName, stoppingToken))
            {
                logger.LogWarning(
                    "{EventName} {EventId} was processed concurrently, duplicate documents may exist",
                    EventName, @event.EventId);
            }
