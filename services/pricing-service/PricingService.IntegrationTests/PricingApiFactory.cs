...
/// RabbitMQ container: <see cref="TestAuthHandler"/> replaces JwtBearer, and the outbox dispatcher
/// (would need RabbitMQ) is removed entirely. These tests only exercise the API + real Postgres
/// persistence, not the external integrations.
/// </summary>
public class PricingApiFactory : WebApplicationFactory<Program>, IAsyncLifetime
{
    private readonly PostgreSqlContainer _postgres = new PostgreSqlBuilder()
        .WithImage("postgres:16-alpine")
        .WithDatabase("pricingservice")
        .WithUsername("pricingservice")
        .WithPassword("pricingservice")
        .Build();
